...
def create_p4_pp_loc_file(program, need_formatting=False):
    ...
    preprocessed = ""
    if program.endswith('-one'):
        for f in os.listdir(d):
            if f.endswith(".p4"):
                fp = os.path.join(d, f)
                res = subprocess.check_output("gcc -x c++ -E -P %s" % fp, shell=True).decode('utf-8')
                preprocessed += res
...
def create_p4_gen_loc_file(program):
    ...
    if program in cc['ncc']['p4'].keys():
        ...
        ncc_command = "%s %s %s -o %s" % (NCLANG, cc['ncc']['p4'][program], ncl_f, gen_f)
        subprocess.check_call([ncc_command], shell=True, stdout=DEVNULL, stderr=DEVNULL)
...
for program in sorted(os.listdir(INPUTS_DIR)):
    if "--cleanup" in sys.argv:
        ...
        if not os.path.exists(NCLANG):
            sys.stderr.write("error: could not find NCLANG at '%s'\n" % NCLANG)
            sys.exit(1)
