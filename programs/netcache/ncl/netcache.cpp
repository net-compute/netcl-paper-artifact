...
enum op_t { GET_RQ, GET_RS, PUT_RQ, PUT_RS, DEL_RQ, DEL_RS, UPD_RQ, UPD_RS };

_net_ uint32_t Cache[CACHELINE_WORDS][CACHE_LINES];
_net_ bool Valid0[CACHE_LINES];
_net_ bool Valid1[CACHE_LINES];
_managed_ uint32_t HitCounts0[CACHE_LINES];
_managed_ uint32_t HitCounts1[CACHE_LINES];
...
_net_ void hit_count(uint32_t cacheline, bitmap_t bitmap) {
  atomic_cond_sadd(&HitCounts0[cacheline], bit_chk(bitmap, 0), 1);
  atomic_cond_sadd(&HitCounts1[cacheline], !bit_chk(bitmap, 0), 1);
}

_net_ void hit_reset(uint32_t cacheline, bitmap_t bitmap) {
  atomic_cond_write(&HitCounts0[cacheline], bit_chk(bitmap, 0), 0);
  atomic_cond_write(&HitCounts1[cacheline], !bit_chk(bitmap, 0), 0);
}

_net_ void heavy_hitter(uint64_t key, bool &hot, uint32_t &count) {
  uint32_t cms[4];
...
_kernel(1) void query(op_t &op, uint64_t key, uint32_t val[CACHELINE_WORDS],
                      bitmap_t &mask, bool &hot, uint32_t &count, bool update) {
  uint16_t cacheline;
  bitmap_t bitmap;
